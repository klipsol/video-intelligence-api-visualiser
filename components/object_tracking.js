	...
	computed: {
		...
		object_track_segments: function () {
			...
			this.indexed_object_tracks.forEach((object_tracks) => {
				if (!(object_tracks.name in segments)) segments[object_tracks.name] = { segments: [], count: 0 };
		...
		json_data.frames.forEach((frame) => {
			const new_frame = {
				box: {
					...
					width:
						((frame.normalized_bounding_box.right || 0) - (frame.normalized_bounding_box.left || 0)) *
						video_width,
					height:
						((frame.normalized_bounding_box.bottom || 0) - (frame.normalized_bounding_box.top || 0)) *
						video_height,
				},
			...
			if (this.frames[index].time_offset > seconds) {
				if (index > 0) {
					...
					const time_delt_ratio =
						(seconds - start_box.time_offset) / (end_box.time_offset - start_box.time_offset);
